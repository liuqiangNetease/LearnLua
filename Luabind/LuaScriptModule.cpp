...
//int tolua_CEGUI_open(lua_State* tolua_S);
    ...
    void LuaScriptModule::executeScriptFile(const String& filename)
    {
        ...
        executeScriptFile_impl(filename,
...
//        CEGUI::Logger::getSingleton().logEvent( "---- Destroying Lua bindings ----" );
    ...
    {
        // set ID string
        d_identifierString = "LuaScriptModule - Lua based scripting module for GUI";
    }
...
//
        ...
        int loaderr = luaL_loadfile(d_state, "main.lua");
        if (loaderr)
        {
...
//        lua_getglobal(d_state, function_name.c_str());
        LuaFunctor::pushNamedFunction(d_state, function_name);
